$(document).ready(function(){
    ...
    $("#eff-start-from-nonate-type3, #eff-start-to-nonate-type3, #eff-end-from-nonate-type3, #eff-end-to-nonate-type3").inputmask("99\\_W99");

    //initialize multiple select2
    $(".select2-nonate-type3").each(function(){
        ...
        if (type == 'print') {
            title_object = {
                customize: function (win) {
                    win.document.title = 'NONATE [Type 3 - Capacity Override]';
                }
            }
        }
        else{
            title_object = {title: 'BRAIN - NONATE [Type 3 - Capacity Override]'};
        }

        let config_object = {
            extend: type,
            filename: function () {
                return file_name_nonate_type_3;
            },
            className: 'd-none buttons-' + type +'-nonate-type-3',
            exportOptions: {
                columns: cols
            },
            ...title_object,
    ...
    var table_type_3 = $(".table-non-type3").DataTable({
        ...
        layout: {
            topStart: "pageLength",
            top2Start: {
                buttons: [exportButtonNonateType3('copy'), exportButtonNonateType3('csv'), exportButtonNonateType3('excel'), exportButtonNonateType3('pdf'), exportButtonNonateType3('print')]
            },
            bottomStart: "info"
        }
    });
    ...
    // -----------------------------------------------------------------------------------EXPORTS------------------------------------------------------------------------------
    $(".btn-export-process-nonate-type3").on("click", function(){
        ...
        if ($.inArray(export_type, ["copy", "print"]) === -1) {
            ...
            table_type_3.button(".buttons-"+export_type+"-nonate-type-3").trigger();
        }
    });
    ...
    $(".btn-export").on("click", function(){
        ...
        if (tab_type == "type-3") {
            if (filename != "") {
                file_name_nonate_type_3 = "BRAIN_"+filename;
                table_type_3.button(".buttons-"+export_type+"-nonate-"+tab_type).trigger();
            }
            else{
                $(".export-error").fadeIn();
            }
...
$(document).ready(function(){
    // -----------------------------------------------------------------------------------------------FILTERS-------------------------------------------------------------------------------
    preFillInputFiltersNonateType3();
    $(".btn-clear-nonate-type3").on("click", function(){
        $(".hwo-inputs-nonate-type3").val('').trigger('change');
        $(".hwo-checks-nonate-type3").prop("checked", false);
    });

    $(".btn-remove-nonate-type3").on("click", function(){
        let curr_url = window.location.href;
        let pos = curr_url.indexOf("&");
        window.location.href = (pos !== -1) ? curr_url.substring(0, pos) : curr_url;
    });

    $('.input-override-cap, .input-hms-count').on('input', function () {
        let value = $(this).val();
    ...
    //for prefilled fyww inputs (enable to inputs if there's a prefilled value and is valid)
    if ($('#eff-start-from-nonate-type3').inputmask('unmaskedvalue').length > 0) {
        $("#eff-start-to-nonate-type3").prop("disabled", false);
    }

    if ($('#eff-end-from-nonate-type3').inputmask('unmaskedvalue').length > 0) {
        $("#eff-end-to-nonate-type3").prop("disabled", false);
    }

    //enabling or disabling and clearing the value of TO fyww inputs if FROM inputs are valid or not (or if user is still typing)
    ...
    $("#eff-start-from-nonate-type3, #eff-start-to-nonate-type3, #eff-end-from-nonate-type3, #eff-end-to-nonate-type3").on("input", function(){
        ...
        $(".btn-set-nonate-type3").prop("disabled", is_disabled);
    });
    ...
    $(".btn-set-nonate-type3").on("click", function(){
        ...
        let param_arr = [];
        $(".hwo-inputs-nonate-type3").each(function(){

            let input_id = $(this).attr("id");
            let input_val = $(this).val();
            
            if (typeof input_val === "object" && input_val.length > 0) {
                var_arr['HW_NAME'] += (input_id == "hw-name-nonate-type3") ? input_val.join(",") : "";
                var_arr['SITE_NUM'] += (input_id == "site-num-nonate-type3") ? input_val.join(",") : "";
                var_arr['RES_AREA'] += (input_id == "res-area-nonate-type3") ? input_val.join(",") : "";
                var_arr['HW_TYPE'] += (input_id == "hw-type-nonate-type3") ? input_val.join(",") : "";
            }
            else if (typeof input_val === "string" && input_val != "") {
                if (input_id == "eff-start-from-nonate-type3") {
                    var_arr['EFF_START'] += input_val;
                }
                if (input_id == "eff-start-to-nonate-type3") {
                    var_arr['EFF_START'] += ","+input_val;
                }
                if (input_id == "eff-end-from-nonate-type3") {
                    var_arr['EFF_END'] += input_val;
                }
                if (input_id == "eff-end-to-nonate-type3") {
                    var_arr['EFF_END'] += ","+input_val;
                }
                if (input_id == "override-cap-min") {
                    var_arr['OVERRIDE_CAP'] += input_val;
                }
                if (input_id == "override-cap-max") {
                    var_arr['OVERRIDE_CAP'] += ","+input_val;
                }
        ...
        $(".hwo-checks-nonate-type3").each(function(){
            let input_id = $(this).attr("id");
            let input_val = $(this).val();
            if ($(this).is(":checked")) {
        ...
        $.each(var_arr, function(key, item){
            if (item != "") {
                if ($.inArray(key, ["OVERRIDE_CAP", "HMS_COUNT"]) !== -1) {
    ...
    function preFillInputFiltersNonateType3(){
        ...
        $.each(params, function(index, item){
            if ($.inArray(index, ["EFF_START", "EFF_END", "OVERRIDE_CAP", "HMS_COUNT"]) !== -1) {
